...
export const handleMethodConCreate = (
  ...
  return (id: number, circlePosition?: { x: number; y: number }) => {
    ...
    if (!pendingAtrCon && circlePosition) {
      ...
      if (pendingAtrCon && id !== pendingAtrCon.attributeID) {

        // check if attribute nodes are connected
        const allIDs = [id, pendingAtrCon.attributeID]
...
export const handleMethodConUpdate = (
  ...
  return (atrConID: number, atrID: number) => {
    if (!pendingAtrCon) {
      ...
      // check if attribute nodes are connected
      const atrNodeID = getAction(atrID)?.id
      const filteredActionCons = parentCons.filter(con => con.actions.find(n => n.id === atrNodeID))
      const filteredAtrCons = cons.filter(atrCon => atrCon.id === atrConID)
